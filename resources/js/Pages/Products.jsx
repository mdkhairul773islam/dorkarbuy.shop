...
export default function Products({ auth, products, categories, filters, fbEventId, fbEventType }) {
    ...
    return (
        <Layout>
            <Head title={__('All Products')} />

            <div className="bg-gray-50 min-h-screen">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
                        <div>
                            <h1 className="text-3xl font-extrabold text-gray-950">{__('All Products')}</h1>
                            <p className="text-gray-500 mt-1">{__('Products desc')}</p>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <option value="clothing">{__('Clothing & Fashion')}</option>
                                    <option value="electronics">{__('Electronics & Gadgets')}</option>
                                    <option value="book">{__('Books')}</option>
                                    <option value="course">{__('Courses & Tutorials')}</option>
                                    <option value="digital">{__('Digital Subscriptions')}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div className="mt-4">
                            ...
                            >
                                <input
                                    ...
                                    placeholder={__('Search Products placeholder')}
                    ...
                    {products?.data?.length === 0 ? (
                        <div className="text-center py-16 bg-white border border-gray-200 rounded-2xl">
                            <p className="text-gray-500 text-lg">{__('No products found')}</p>
                        </div>
                    ) : (
                        <>
